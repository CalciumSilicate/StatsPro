...
import os
import shutil
from mcdreforged.plugin.server_interface import ServerInterface, Info
...
players_stats = {}
permissions_needed_num = ['guest', 'user', 'helper', 'admin', 'owner'].index(permissions_needed)
default_config = {
    'gen_list': {
        'sum': {},
        'record': {},
    ...
    'presuppositions': {
        'default': {
            'list': {
                'used': [
                    'diamond_axe', 'diamond_sword', 'diamond_pickaxe', 'diamond_shovel', 'iron_axe', 'iron_sword',
                    'iron_pickaxe', 'iron_shovel', 'golden_axe', 'golden_sword', 'golden_pickaxe', 'golden_shovel',
                    'wooden_axe', 'wooden_sword', 'wooden_pickaxe', 'wooden_shovel', 'stone_axe', 'stone_sword',
...
'''.format(plugin_name, '§l' + prefix + '§r', PLUGIN_METADATA['version'], '§d§l' + prefix + '§r')
...
'''.format(plugin_name, '§l' + prefix + '§r', PLUGIN_METADATA['version'], '§d§l' + prefix + '§r', permissions_needed)
help_messages = [help_message, help_message_2, help_message_3]
flag_auto = False


def simple_text(message, text='', command='', action=RAction.run_command):
    r_text = RText(message).set_hover_text(text).set_click_event(RAction.suggest_command, '')
    if command:
...
def init_presuppositions_abbr():
    ...
    for pre_name, pre_value in temp.items():
        ...
        if is_1_16 and pre_name == 'default':
            for i in ['netherite_pickaxe', 'netherite_axe', 'netherite_sword', 'netherite_shovel']:
        ...
        for x in range(len(abbr_list)):
            presuppositions[pre_name]['list'][whole_list[x][:whole_list[x].index('.')]][item_list[x]] = abbr_list[x]
...
def print_msg(server: ServerInterface, info: Info, msg, msg_prefix: str = '§r[§b{}§r]'.format(plugin_name),
              is_tell=True) -> None:
...
def get_player_stats(arg: str, is_refresh: bool = False) -> dict or None:
    if is_refresh:
        ...
        if players_stats:
            try:
                return players_stats[arg] if arg in list(players_stats.keys()) else players_stats[get_uuid(uuid=arg)]
            except KeyError:
                return None
        else:
...
def print_rank(server, info, rank_list: dict, title: str = None, rank_amount: int = 15):
    i = 0
    sorted_list = dict(list(sorted(rank_list.items(), key=lambda x: x[1], reverse=True))[:rank_amount])
    max_len = max(len(x) for x in sorted_list.keys())
    if title is not None:
        print_msg(server, info, title.replace('%s', str(sum(rank_list.values()))), '', is_tell=False)
    color_list = ['§6', '§b', '§3']
    for name, value in sorted_list.items():
        i += 1
        name = name.replace('minecraft:', '')
        print_msg(server, info, '{}#{}{} {}{}{}{}'.format(
            color_list[i - 1] if i <= len(color_list) else '',
            i, ' ' * (len(str(rank_amount)) - len(str(i))), name,
               ' ' * (max_len - len(name)),
               ' ' * (10 - len(str(value))), value
        ), '', is_tell=False)
...
def query(server, info, arg: str, cls: str = None, item: str = None, rank_amount=100):
    ...
    if arg != '*':
        ...
        if cls and item:
            ...
            scores = dict(((k, v[t_item]) if t_item in v else ...) for k, v in scores.items())
    arg = convert_uuid_name(arg)
    print(arg)
    if (cls and not item) or (item and not cls):
        try:
            print_rank(
                server, info, rank_list=scores,
                title='{}的[{}§r]前{}名如下, 总和为%s'.format(
                    arg, '§e' + cls if cls else '§b' + item, len(scores) if len(scores) < rank_amount else rank_amount),
                rank_amount=rank_amount
            )
        except TypeError:
            print_msg(server, info, '出错了')
            print_msg(server, info, '可能的原因:\n1. 不存在名为{0}的玩家\n2. {0}的{1}为空\n3. {1}拼写错误{2}'.format(
                arg, cls if cls else item,
                '' if cls and item else '\n4. {}不属于{}'.format(cls if cls else item, 'cls' if cls else 'item')), '')
    else:
        print_msg(server, info,
                  '{}的[§e{}§r.§b{}§r]的值为: {}'.format(convert_uuid_name(arg), cls, item, scores), '', is_tell=False)
            ...
            try:
                if cls and not item:
                    ...
                    for i, v in stats['stats'].items():
                        try:
                            output_list['{}.{}'.format(name, i.replace('minecraft:', ''))] = v[con_prefix(item)]
                        except KeyError:
                            continue
                else:
                    output_list[name] = stats['stats'][con_prefix(cls)][con_prefix(item)]
            except KeyError:
                continue
    return output_list


def rank(server, info, cls: str = None, item: str = None, rank_amount=15, bot=False, is_refresh=False):
    if is_refresh or not players_stats:
        get_players_stats()
        return rank(server, info, cls, item, rank_amount, bot, False)
    rank_list = get_list(cls, item, bot)
    try:
        print_rank(server, info, rank_list, title='[{}§r]的总和为%s, 前{}名如下:'.format(
            '§e{}§r.§b{}§r'.format(cls, item) if cls and item else ('§e' + cls if cls else '§b' + item),
        ...
        print_msg(server, info, '可能的原因:\n1. {0}拼写错误\n2. {0}为空{1}'.format(
            ...
            '' if cls and item else '\n3. {}不属于{}'.format(cls if cls else item, 'cls' if cls else 'item')
        ), '')


def set_display(server, arg=scoreboard_name):
    return server.execute('scoreboard objectives setdisplay sidebar{}'.format((' ' + arg) if arg else arg))


def scoreboard_builder(server, info, cls, item, title: str = None, inner_name: str or None = None, bot=False):
    ...
    for name, value in value_list.items():
        server.execute('scoreboard players set {} {} {}'.format(name, inner_name, value))
    return value_list
    ...
    for key, value in output_stats.items():
        for item in value.keys():
            output_stats[key][item] = sum(
                (stats[key][item] if key in stats and item in stats[key] else 0) for stats in stats_list)
    return output_stats if data_version is None else {'stats': output_stats, 'DataVersion': data_version}
...
def sum_scoreboard_command(presupposition='default'):
    ...
    abbr_list = []
    commands = {'creating': [], 'removing': [],
                'names': {'true': [], 'dummy': []}}
    for cls, value in pre['list'].items():
        for item, abbr in value.items():
            commands['removing'].append('scoreboard objectives remove {}_{}'.format(pre['prefix_true'], abbr))
    ...
    for abbr in abbr_list:
        commands['creating'].append('scoreboard objectives add {}_{} dummy'.format(pre['prefix_dummy'], abbr))
        commands['removing'].append('scoreboard objectives remove {}_{}'.format(pre['prefix_dummy'], abbr))
        commands['names']['dummy'].append('{}_{}'.format(pre['prefix_dummy'], abbr))
    commands['creating'].append('scoreboard objectives setdisplay sidebar {}_total'.format(pre['prefix_dummy']))
    commands['creating'].append(
        'scoreboard objectives modify {}_total displayname {}'.format(pre['prefix_dummy'],
                                                                      json.dumps({'text': pre['name']})))
    return commands
...
def datapacks_command(presupposition='default'):
    ...
    def cmd(target, operation, source):
        return 'execute as @a run scoreboard players operation @s {} {} @s {}'.format(target, operation, source)
...
def switch_sum_scoreboard(server, info, presupposition='default', is_opening=True, bot=False, is_call=False, i_flag=True):
    ...
    function_path: str = os.path.join(datapack_path, 'data', plugin_name.lower(), 'functions')
    tags_path: str = os.path.join(datapack_path, 'data', 'minecraft', 'tags', 'functions')
    if os.path.exists(datapack_path):
        ...
        if is_opening:
            ...
            if not is_call:
                print_msg(server, info, '已成功关闭加和计分板({})'.format(presupposition))
    elif is_opening:
        value_list = {}
        for cls, value in pre['list'].items():
            if cls not in value_list:
                value_list[cls] = {}
            for item, abbr in value.items():
                value_list[cls][item] = scoreboard_builder(server, info, cls=cls, item=item, bot=bot,
        ...
        for player, value in out_list.items():
            server.execute('scoreboard players set {} {}_{} {}'.format(player, pre['prefix_dummy'], 'total', value))
        ...
        with open(os.path.join(tags_path, 'tick.json'), 'w') as tick:
            tick.write(json.dumps({'values': ['{}:tick'.format(plugin_name.lower())]}, indent=4))
        with open(os.path.join(datapack_path, 'pack.mcmeta'), 'w') as meta:
            meta.write(json.dumps({'pack': {'pack_format': 4, 'description': ''}}, indent=4))
        server.execute('reload')
        if not is_call:
    ...
    if is_creating:
        for pre in presuppositions.values():
            if kwargs['prefix_dummy'] in [pre['prefix_dummy'], pre['prefix_true']]:
                print_msg(server, info, '无实义计分板前缀({}_)与名为{}的预设重名'.format(kwargs['prefix_dummy'], pre['name']))
                return
            elif kwargs['prefix_dummy'] in [pre['prefix_dummy'], pre['prefix_true']]:
                print_msg(server, info, '实义计分板前缀({}_)与名为{}的预设重名'.format(kwargs['prefix_true'], pre['name']))
                return
        presuppositions[presupposition] = {
            'list': {}, 'name': kwargs['name'], 'prefix_dummy': kwargs['prefix_dummy'],
            'prefix_true': kwargs['prefix_true']
        }
        print_msg(server, info, '成功创建名为{}的预设，无实义计分板、实义计分板的前缀分别为{}_、{}_'.format(
...
def presupposition_change(server, info, presupposition, cls='', item='', is_adding=True, is_del_all=False):
    global presuppositions
    ...
    if is_del_all:
        ...
        print_msg(server, info, '成功删除名为{}的预设中所有计分项'.format(presupposition))
    else:
        for pre_cls, pre_items in pre.items():
            for i in pre_items:
                if pre_cls == cls and i == item:
                    if is_adding:
                        print_msg(server, info, '预设{}中已有{}.{}计分项'.format(presupposition, cls, item))
                        return None
                    else:
                        del pre[pre_cls][pre_items]
                        print_msg(server, info, '成功删除预设{}中的{}.{}'.format(presupposition, cls, item))
                        return True
        if is_adding:
            if cls not in pre:
                pre[cls] = {}
            pre[cls][item] = ''
            print_msg(server, info, '成功在{}预设中添加{}.{}'.format(presupposition, cls, item))
    init_presuppositions_abbr()
    write_config()
...
def gen_file(server, info: Info = None, **kwargs):
    ...
    if mode == 'sum':
        ...
        output_list[path] = json.dumps(sum_stats(
            *list((value if players is None or name in players else {}) for name, value in players_stats.items())),
            indent=4)
    else:
        ...
        if mode == 'sum':
            ...
            with open(first, 'r') as f:
                with open(second, 'r') as s:
                    output_list[path] = json.dumps(minus_stats(json.load(f), json.load(s)))
        elif mode == 'record':
            path_list = {}
    ...
    cfg['gen_list'][kwargs['mode']][time_now] = {
        'time': time_now, 'name': time_now + note,
        'note': note.replace('#', ''),
        'path': path, 'abs_path': os.path.abspath(path).replace('\\', '/')
    }
    ...
    if info is None:
        return
    print_msg(server, info, '成功生成{}文件：\n时间： {}，\n名称： {}， \n注释： {}，\n路径： {}，\n绝对路径： {}'.format(
        kwargs['mode'],
        *('§b' + str(x).replace('\\', '/') + '§r' for x in list(cfg['gen_list'][kwargs['mode']][time_now].values()))).
...
def merge(server, info, **kwargs):
    ...
    if mode == 'add':
        ...
        if not input_list or not output_player:
            ...
            return
        stats = sum_stats(*list(players_stats[n] for n in
                                (input_list if output_player in input_list else [*input_list, output_player])), )
        with open(os.path.join(stats_path, get_uuid(name=output_player) + '.json'), 'w') as file:
            file.write(json.dumps(stats))
        for x in input_list:
            if x != output_player:
                if get_uuid(name=x) + '.json' in os.listdir(stats_path):
                    os.remove(os.path.join(stats_path, get_uuid(name=x) + '.json'))
        print_msg(server, info, '成功将{}的数据信息合并到{}'.format('，'.join(input_list), output_player))
    write_config()
...
def view_list(server: ServerInterface, info: Info):
    ...
    for pre_name, value in presuppositions.items():
        r_text_list = RTextList(simple_text(
            '预设名：§b{}§r， 实义/无实义计分板前缀：§e{}§r / §c{}§r，'.format(pre_name, value['prefix_true'], value['prefix_dummy']) +
            '计分板显示名称：{}， 详情子计分项：'.format(value['name']), '点击查看详情', '{} sum view {}'.format(
                prefix, pre_name
            )))
...
def view_list_detail(server: ServerInterface, info: Info, arg='default'):
    ...
    for cls, v in value['list'].items():
        print_msg(server, info, '§l' + cls.title() + '§r' + ': §b{}§r项'.format(len(v)), '')
        for item, abbr in v.items():
            print_msg(server, info, ' - {}{} : {}'.format(item, ' ' * (max_len - len(item)), abbr), '')
...
def gen_list(server: ServerInterface, info: Info, arg=''):
    if arg == 'all':
        ...
        for i in cfg['gen_list'][arg].values():
            print_msg(server, info, RTextList(
                simple_text('时间（点击复制）: §b{}§r'.format(i['time']), command=i['time'],
                            action=RAction.suggest_command, text='点击复制'),
                '， 名称：§c{}§r， 注释：§7{}§r， '.format(i['name'], i['note']),
                simple_text('§e[路径（悬浮查看或点击复制）§r]', text=i['path'], command=i['path'],
                            action=RAction.suggest_command),
                simple_text('[§c绝对路径（同上）§r]', text=i['abs_path'], command=i['abs_path'],
                            action=RAction.suggest_command),
                simple_text(' [§c删除§r]', command='{} gen del {} {}'.format(prefix, arg, i['time']))), '')


def merge_list(server: ServerInterface, info: Info):
    print_msg(server, info, 'input玩家列表如下：', '')
    for i in cfg['merge_list']['input']:
        print_msg(server, info, RTextList(i, simple_text('    [§c×§r]', command='{} merge del {}'.format(prefix, i))),
                  '')
    print_msg(server, info, 'output玩家是：', '')
    if cfg['merge_list']['output']:
        print_msg(server, info, RTextList(cfg['merge_list']['output'], simple_text(
            '    [§er§r]', command='{} merge set '.format(prefix), text='点击重设', action=RAction.suggest_command)), '')


@new_thread
def auto(server: ServerInterface):
    global flag_auto
    while server.is_server_running:
        flag_auto = True
        gen_file(server, mode='sum')
        gen_file(server, mode='record')
        time.sleep(7200)
...
def on_user_info(server: ServerInterface, info: Info):
    ...
    cmd = cmd.replace('-bot', '').replace('true', 'True').replace('false', 'False').split()
    cmd[0] = cmd[0].replace('！', '!')
    len_cmd = len(cmd)
    ...
    # !!sp help [PageIndex]
        ...
        if page_index != 1:
            r_text_list.append(simple_text('[§b←上一页§r]', text='{} help {}'.format(prefix, page_index - 1),
                                           command='{} help {}'.format(prefix, page_index - 1),
                                           action=RAction.run_command))
        if page_index != 3:
            r_text_list.append(simple_text('{}[§c下一页→§r]'.format('   ' if page_index != 1 else ''
    ...
    # !!sp query
        ...
        if third_cmd in third_list:
            ...
            # !!sp query cls <Player> <Cls> [RankAmount]
                ...
                query(server, info, player, cls=cmd[4], rank_amount=int(cmd[5]) if len_cmd == 6 else 99)

            # !!sp query item <Player> <Item> [RankAmount]
            elif len_cmd in [5, 6] and third_cmd == third_list[2]:
                query(server, info, player, item=cmd[4], rank_amount=int(cmd[5]) if len_cmd == 6 else 99)
    ...
    # !!sp rank
        ...
        if third_cmd in third_list:
            ...
            if len_cmd in [5, 6] and third_cmd == third_list[0]:
                rank(server, info, cmd[3], cmd[4], rank_amount=int(cmd[5]) if len_cmd == 6 else 15, bot=bot)

            # !!sp rank cls <Cls> [RankAmount] [-Bot]
            elif len_cmd in [4, 5] and third_cmd == third_list[1]:
                rank(server, info, cls=cmd[3], rank_amount=int(cmd[4]) if len_cmd == 5 else 99, bot=bot)

            # !!sp rank item <Item> [RankAmount] [-Bot]
            elif len_cmd in [4, 5] and third_cmd == third_list[2]:
                rank(server, info, item=cmd[3], rank_amount=int(cmd[4]) if len_cmd == 5 else 99, bot=bot)
    ...
    # !!sp scoreboard <Cls> <Item> [DisplayName]
        ...
        save(server, False)
        scoreboard_builder(server, info, cmd[2], cmd[3], cmd[4] if len_cmd == 5 else None, bot=bot)
        set_display(server)
        print_msg(server, info,
                  '已创建{}计分板，内部ID为{}'.format(cmd[4] if len_cmd == 5 else '§e{}§r.§b{}§r'.format(cmd[2], cmd[3]),
                                            scoreboard_name))
    ...
    # !!sp sum
        ...
        third_list = ['make', 'clear', 'create', 'remove', 'add', 'del', 'del_all', 'remove_all', 'list', 'view']
        if third_cmd in third_list:

            # !!sp sum remove/add/del/del_all [...]
            if len_cmd in [4, 6] and third_cmd in third_list[3:7]:
                save(server)
                switch_sum_scoreboard(server, info, cmd[3], is_opening=False, is_call=True)
            ...
            if len_cmd in [3, 4] and third_cmd in third_list[:2]:
                if cmd[2] == third_list[0]:
                    save(server)
                switch_sum_scoreboard(server, info, cmd[3] if len_cmd == 4 else 'default', bot=bot,
            ...
            # !!sp sum create <PresuppositionName> [PrefixTrue] [PrefixDummy] [DisplayName]
                ...
                presupposition_control(server, info, presupposition_name, is_creating=True, prefix_true=prefix_true,
                                       prefix_dummy=prefix_dummy, name=display_name)
            ...
            # !!sp sum add/del <PresuppositionName> <Cls> <Item>
                ...
                presupposition_change(server, info, cmd[3], cmd[4], cmd[5], is_adding=third_cmd == third_list[4])
    ...
    # !!sp gen
        ...
        if third_cmd in third_list:
            ...
            if len_cmd in [3, 4] and third_cmd in third_list[:2]:
                gen_file(server, info, mode=third_cmd, note=cmd[3] if len_cmd == 4 else '')
            ...
            # !!sp gen minus sum/record <Time1> <Time2>
                ...
                gen_file(server, info, mode=third_cmd, minus_mode=cmd[3], first=cmd[4], second=cmd[5])
